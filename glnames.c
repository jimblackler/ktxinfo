...
const char* name_from_enum(unsigned int id) {
  const Pair* pair = gl_enums;
  while (1) {
    if (pair->id > id) {
      return "<unknown>";
    }
...
void printf_glenum(unsigned int id) {
  const char* name = name_from_enum(id);
  printf("0x%04X %s", id, name);
}